...
const canRenderProtectedData = () => ['admin', 'officer', 'viewer'].includes(getSupabaseRole());
...
const preparePrintHospitalHeader = () => {
  ...
  const title=root.querySelector('h1')?.textContent?.trim() || target.querySelector('h1,h2')?.textContent?.trim() || 'รายงานระบบเฝ้าระวังโรค';
...
const enhanceAdminUserManagement = () => {
  ...
  section.innerHTML = `<div class="panel-top"><div><h2>จัดการบัญชีและสิทธิ์ผู้ใช้งาน</h2><small>เฉพาะ ADMIN: ตรวจชื่อ–สกุล อีเมล และเบอร์โทร พร้อมกำหนดสิทธิ์และสถานะ รออนุมัติ / เปิดใช้งาน / ระงับบัญชี / ลบบัญชี ได้จากตารางเดียว</small></div><button type="button" class="secondary" data-refresh-admin-users>รีเฟรช</button></div><div class="responsive-table"><table><thead><tr><th>รายละเอียดผู้ใช้งาน</th><th>สิทธิ์ปัจจุบัน</th><th>สถานะ</th><th>เข้าสู่ระบบล่าสุด</th><th>จัดการ</th></tr></thead><tbody data-admin-users><tr><td colspan="5">กำลังโหลดบัญชี...</td></tr></tbody></table></div><p class="scope-note">รหัสผ่านเป็นข้อมูลเข้ารหัส จึงไม่สามารถแสดงหรือกู้คืนได้ ให้ผู้ใช้ใช้ “ลืมรหัสผ่าน” เพื่อกำหนดรหัสผ่านใหม่อย่างปลอดภัย การเปลี่ยนสิทธิ์มีผลเมื่อผู้ใช้เข้าสู่ระบบใหม่หรือรีเฟรช session ครั้งถัดไป เพื่อให้ JWT รับ app metadata ชุดใหม่</p>`;
...
const canEditInvestigation = item => {
  const role=getSupabaseRole();
...
const renderHistory = keyword => {
  ...
  if(!rows) return;
  const cases=JSON.parse(localStorage.getItem('ndss-investigations') || '[]');
  const search=(keyword ?? root.querySelector('[data-history-search]')?.value ?? '').toLowerCase();
  const disease=root.querySelector('[data-history-disease]')?.value || '';
  const from=root.querySelector('[data-history-from]')?.value || '';
  const to=root.querySelector('[data-history-to]')?.value || '';
  const canDelete=getSupabaseRole()==='admin';
  const filtered=cases.map((item,index)=>({...item,index})).filter(item=>{
    const day=(item.createdAt || '').slice(0,10);
    return [item.patient,item.hn,item.disease,item.location].join(' ').toLowerCase().includes(search)
      && (!disease || item.disease===disease) && (!from || day>=from) && (!to || day<=to);
  });
...
document.addEventListener('submit', async event => {
  ...
  const previous=editingCaseIndex === null ? {} : records[editingCaseIndex] || {};
  if(editingCaseIndex !== null && !canEditInvestigation(previous)) {
    showToast('OFFICER แก้ไขได้เฉพาะเคสที่ตนสร้างหรือได้รับมอบหมาย', 'error');
    return;
  }
  let saved={
    ...previous,
    ...value,
    location:value.location || [value.subdistrict,value.district,value.province].filter(Boolean).join(' '),
    disease:event.target.querySelector('[data-disease]').value,
    createdAt:editingCaseIndex === null ? new Date().toISOString() : previous.createdAt,
    createdBy:editingCaseIndex === null ? (getSupabaseUser()?.sub || '') : previous.createdBy,
    assignedTo:editingCaseIndex === null ? (getSupabaseUser()?.sub || '') : previous.assignedTo,
    updatedAt:new Date().toISOString(),
  };
  if (editingCaseIndex === null) records.push(saved); else records[editingCaseIndex]=saved;
  const savedIndex=editingCaseIndex === null ? records.length-1 : editingCaseIndex;
  localStorage.setItem('ndss-investigations',JSON.stringify(records));
  try {
    saved=await syncInvestigationCase(saved);
    records[savedIndex]=saved;
    localStorage.setItem('ndss-investigations',JSON.stringify(records));
  } catch(error) {
    console.warn('Investigation case remains available locally until it can be synced.', error);
    showToast('บันทึกในเครื่องแล้ว แต่ยังไม่ซิงก์ฐานข้อมูลกลาง', 'info');
  }
  window.dispatchEvent(new Event('ndss-cases-updated'));
  editingCaseIndex=null;
  root.querySelector('[data-form-modal]').hidden=true;
