export function shell(content) {
  ...
  return `<input id="mobile-sidebar-state" class="mobile-sidebar-state" type="checkbox" aria-label="เปิดหรือปิดเมนูหลัก" /><aside class="sidebar command-sidebar"><button class="mobile-sidebar-close" type="button" onclick="var s=document.getElementById('mobile-sidebar-state');s.checked=false;s.dispatchEvent(new Event('change',{bubbles:true}))" aria-label="ปิดเมนูหลัก">✕ <span>ปิดเมนู</span></button><div class="sidebar-identity"><img src="./public/assets/naradhiwas-hospital-logo.jpg" alt="โลโก้โรงพยาบาล" /><div><strong>ศูนย์บัญชาการข้อมูลโรค</strong><small>โรงพยาบาลนราธิวาสราชนครินทร์</small></div></div><div class="system-name"><b>NDSS</b><small>ระบบเฝ้าระวัง สอบสวน และวิเคราะห์<br/>สถานการณ์โรคระดับพื้นที่</small></div><nav id="mobile-primary-navigation" aria-label="เมนูหลัก">${menu}</nav><div class="sync"><b><em>●</em> สถานะการเชื่อมต่อ</b><span>ข้อมูลจากอุปกรณ์นี้</span><small>เชื่อมฐานข้อมูลกลางได้จากเมนูตั้งค่า</small></div><button class="sidebar-signout" type="button" data-supabase-signout>↪ ออกจากระบบ</button></aside><main><header class="top-header command-header"><button class="mobile-nav-toggle" type="button" onclick="var s=document.getElementById('mobile-sidebar-state');s.checked=!s.checked;s.dispatchEvent(new Event('change',{bubbles:true}))" aria-controls="mobile-primary-navigation" aria-expanded="false">☰ <span>เมนู</span></button><div class="header-context"><b>ศูนย์บัญชาการระบาดวิทยา</b><small>ข้อมูลจริงจาก รง.506 และแบบสอบสวนโรคออนไลน์</small></div><div class="header-actions"><button class="header-import" type="button" data-open-506-import>＋ นำเข้า Excel</button><button class="notification" type="button" aria-label="การแจ้งเตือน">♧<i>0</i></button><div class="profile-chip"><span>👩‍⚕️</span><div><b>ผู้ใช้งานระบบ</b><small>NDSS Narathiwat</small></div><em>⌄</em></div></div></header><div id="module-root">${content}</div></main>`;
}
